...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- ================================================================ -->
    <xsl:variable name="originaltales" as="document-node()+" select="
        collection('../xmlOriginalMarkup?select=*.xml')"/>
    <xsl:variable name="max_height" as="xs:double" select="800"/>
    <xsl:variable name="spacing" as="xs:double" select="200"/>
    <xsl:variable name="max_width" as="xs:double" select="($spacing) * count($originaltales//story)"/>
    <!-- ================================================================ -->
    <!-- Templates                                                        -->
    <!-- ================================================================ -->
    <xsl:template name="xsl:initial-template">
        <xsl:result-document href="../SVG/originalgraph.svg"
            ...
            <svg height="{$max_height + 300}" width="{$max_width + 400}"
                ...
                <xsl:for-each-group select="$originaltales" group-by="descendant::story">
                    
                    <!-- ==================================================== -->
                    <!-- Create axis labels                                   -->
                    <!-- ==================================================== -->
                    <text x="{$max_width div 2}" y="-{$max_height + 50}" text-anchor="middle"
                        font-size="18">Female Autonomy in Original Fairytales</text>
                    ...
                    <!-- ==================================================== -->
                    <xsl:apply-templates select="$originaltales/story"/>
                </xsl:for-each-group>
                <!-- ==================================================== -->
