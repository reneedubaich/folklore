<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- ================================================================ -->
    <xsl:variable name="max_height" as="xs:double" select="400"/>
    <xsl:variable name="spacing" as="xs:double" select="100"/>
    <xsl:variable name="max_width" as="xs:double" select="($spacing) * count(//story)"/>
    <!-- ================================================================ -->
    <!-- Templates                                                        -->
    <!-- ================================================================ -->
    <xsl:template match="/">
    ...
    <xsl:template match="story">
        ...
        <rect x='{$xpos}' y='-{$controlledCount * 40}' width="20" height="{$controlledCount * 40}" fill="red"/>
        <!-- ============================================================ -->
        <!-- Create freed bars                                               -->
        <!-- ============================================================ -->
        <xsl:variable name="freeCount" as="xs:double" select="count(//autonomy[@auto_type='free'])"/>
        <rect x='{$xpos + 20}' y='-{$freeCount * 40}' width="20" height="{$freeCount * 40}" fill="blue"/>
        <!-- ============================================================ -->
        <!-- Create story label                        -->
        <!-- ============================================================ -->
